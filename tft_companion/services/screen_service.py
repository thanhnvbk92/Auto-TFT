...
def read_screen(output_dir: Path) -> ScreenReadResult:
    ...
    result = read_image(image_path, source="pc")
    ocr_seconds = time.perf_counter() - ocr_started
    return _with_timing(result, capture_seconds, ocr_seconds, time.perf_counter() - started)
...
def read_android_screen(output_dir: Path, device_id: str = "") -> ScreenReadResult:
    ...
    try:
        ...
        return ScreenReadResult(
            ...
            source="android",
            error=str(exc),
            timing=ReadTiming(capture_seconds=capture_seconds, total_seconds=time.perf_counter() - started),
        )
...
def _with_timing(
    ...
    return ScreenReadResult(
        ...
        error=result.error,
        timing=ReadTiming(capture_seconds, ocr_seconds, total_seconds),
    )
...
def read_tft_mobile_state(image_path: Path, read_shop: bool = False) -> tuple[dict[str, str], str]:
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        gold_future = executor.submit(_read_mobile_gold, image.copy())
...
def _read_mobile_gold(image: Image.Image) -> str:
    # Shop screens show gold near the top-right, combat screens also keep
    # the larger gold button at bottom-right. Try both and use the first
    # clean numeric result.
    candidate_boxes = [
        ((1890, 0, 2025, 80), None),
        ((2485, 1100, 2565, 1205), 180),
        ((2490, 1090, 2570, 1210), 180),
    ]
    for box, threshold in candidate_boxes:
        text = _ocr_region(
            image,
            box,
            psm=10,
